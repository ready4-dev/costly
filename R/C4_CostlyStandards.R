#' CostlyStandards
#' 
#' Dataset (and metadata) defining the allowable values of specified variables.
#' 
#' @include C4_CostlySource.R
#' @slot args_ls Arguments (a list)
#' @slot Ready4useDyad_r4 A dataset and data dictionary pair. (a ready4 S4)
#' @slot include_chr Include (a character vector)
#' @slot label_1L_chrs  (an instance of the character class)
#' @slot match_1L_chr Match (a character vector of length one)
#' @slot dissemination_1L_chr Dissemination (a character vector of length one)
#' @name CostlyStandards-class
#' @rdname CostlyStandards-class
#' @export CostlyStandards
#' @exportClass CostlyStandards
CostlyStandards <- methods::setClass("CostlyStandards",
contains = "CostlySource",
slots = c(args_ls = "list",Ready4useDyad_r4 = "Ready4useDyad",include_chr = "character",label_1L_chrs = "character",match_1L_chr = "character",dissemination_1L_chr = "character"),
prototype =  list(args_ls = list()))


methods::setValidity(methods::className("CostlyStandards"),
function(object){
msg <- NULL
if (is.null(msg)) TRUE else msg
})
