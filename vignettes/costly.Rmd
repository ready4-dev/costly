---
title: "costly"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{costly}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

```{r setup}
library(costly)
```
This development version of the costly package has been made available as part of the process of testing and documenting the package. 
Currently there are no vignettes available.

A brief overview of some of the intended use of costly tools is provided in the following article:

- [Standardise Variable Values With Fuzzy Logic And Correspondence Tables](V_01.html)

The steps detailed in that vignette can more succinctly be accomplished with the following code (a country variable specific implementation).

```{r}
X <- CostlyCountries() # Create CostlyCountries module instance
X <- renew(X, type_1L_chr = "default") # Specify the dataset to be standardised, the standards for country names and partial correspondences.
X <- renew(X, "jw", type_1L_chr = "slot", what_1L_chr = "logic") # Specify fuzzy logic type to use to supplement partial correspondences.
X <- renew(X, T, type_1L_chr = "slot", what_1L_chr = "force") 
X <- ratify(X) # Create a standardised dataset and validate it against the standards dataset.
```



```{r echo=FALSE}
# Still buggy - need to fix
#Not yet represented as a vignette is the logic to standardise variable values using lookup tables. The basic logic (specific to currency variables) is as follows.
Y <- CostlyCountries()
Y <- renew(Y,
           new_val_xx = add_default_currency_seed(Y@CostlySeed_r4, include_1L_chr = "Country"), 
           what_1L_chr = "seed")
Y <- renew(Y, "jw", type_1L_chr = "slot", what_1L_chr = "logic") 
Y <- renew(Y, new_val_xx = make_country_correspondences("currencies"), 
           type_1L_chr = "final", what_1L_chr = "correspondences")
Y <- ratify(Y)
Z <- CostlyCurrencies()
Z <- renew(Z, new_val_xx = add_default_currency_seed(Z@CostlySeed_r4,
                                                     Ready4useDyad_r4 = Y@results_ls$Country_Output_Lookup), 
           what_1L_chr = "seed")
Z <- ratify(Z, type_1L_chr = "Lookup")
Z <- renew(Y, T, type_1L_chr = "slot", what_1L_chr = "force") 
Z <- ratify(Z, type_1L_chr = "Lookup")
```

```{r echo=FALSE}
# renew - Make "slot" default, move what to third position
# ratify - Make null default new val
```

